...
if(isset($_GET['id'])){
	...
	$cek = mysqli_query($conn, "SELECT * FROM penulis WHERE id_penulis='$id'") or die(mysqli_error($conn));

	//jika query menghasilkan nilai > 0 maka eksekusi script di bawah
	if(mysqli_num_rows($cek) > 0){
		//query ke database DELETE untuk menghapus data dengan kondisi id=$id
		$del = mysqli_query($conn, "UPDATE penulis SET is_delete=1 WHERE id_penulis='$id'") or die(mysqli_error($conn));
		if($del){
