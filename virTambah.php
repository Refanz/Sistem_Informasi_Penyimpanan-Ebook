	...
	<div class="container" style="margin-top:20px">
		...
		if(isset($_POST['submit'])){
			...
			if(mysqli_num_rows($cek) == 0){
				$buku = mysqli_query($conn, "INSERT INTO buku(id_buku, judul, tahun, jml_unduhan, bahasa, book_file,kd_penerbit, kd_penulis)
                                     VALUES('$id_buku', '$judul', '$tahun', '$jml_unduhan', '$bahasa', '$book_file_name', '$kd_penerbit', '$kd_penulis')") 
                                     or die(mysqli_error($conn));
									 
				move_uploaded_file($book_file_tmp_name, $path);
				
				if($buku){
					echo '<script>alert("Berhasil mengajukan data buku, silahkan menunggu untuk disetujui oleh Admin."); document.location="virTambah.php";</script>';
				}else{
		...
		<form action="virTambah.php" method="post" enctype="multipart/form-data">
    ...
    <div class="container" style="margin-top:20px">
		...
		if(isset($_POST['submit_penulis'])){
			...
			if(mysqli_num_rows($cek) == 0){
				...
				if($penulis){
					echo '<script>alert("Berhasil menambahkan data."); document.location="virTambah.php";</script>';
				}else{
					echo '<div class="alert alert-warning">Gagal melakukan proses upload data.</div>';
				}
			}else{
		...
		<form action="virTambah.php" method="post">
			<div class="form-group row">
